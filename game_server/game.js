...
const gameObj = {
  playersMap: new Map(), // players
  itemsMap: new Map(), // missile item
  airMap: new Map(), // oxygen to breathe 
  NPCMap: new Map(),
  addingNPCPlayerNum: 9, // 10 players can play
...
const gameTicker = setInterval(() => {
  NPCMoveDecision(gameObj.NPCMap); // NPC action selection
...
function checkGetItem(playersMap, itemsMap, airMap, flyingMissilesMap) {
  for (let [hashKey, playerObj] of playersMap) {
    ...
    for (let [itemKey, itemObj] of itemsMap) { 
      ...
      if( // if close distance, then get item
    ...
    for (let [airKey, airObj] of airMap) {
      ...
      if( // if close distance, then get air
        ...
        if(playerObj.airTime + gameObj.addAir > 99){
...
module.exports = {
  newConnection, getMapData, updatePlayerDirection, missileEmit,disconnect
};